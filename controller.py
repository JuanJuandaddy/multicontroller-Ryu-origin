...
from ryu.ofproto import ofproto_v1_0 as cbench_compatible_version
...
SW_TO_SW_PRIORITY=settings.SW_TO_SW_PRIORITY

IPV6_PRIORITY=settings.IPV6_PRIORITY

SW_TO_HOST_PRIORITY=settings.SW_TO_HOST_PRIORITY

TABLEMISS_PRIORITY=settings.TABLEMISS_PRIORITY

PERFORMANCE_STATISTIC_ECHO=settings.PERFORMANCE_STATISTIC_ECHO

IDLE_TIME_OUT, HARD_TIME_OUT = settings.IDLE_TIME_OUT, settings.HARD_TIME_OUT
class Controller(app_manager.RyuApp):
    OFP_VERSIONS = [ofp_version.OFP_VERSION,cbench_compatible_version.OFP_VERSION]
    _CONTEXTS={
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.monitor_spawn()
        # =========================控制器负载========================
        self.switches_pktin_load={} ##(dpid, {"new":xx, "old":xx, "speed":xx,"new_size":xx,"odl_size":xx,"size":xx})

        self.topo_load=0

        self.pkt_in_num=0

        self.old_pkt_in_num=0
    ...
    def _discover(self):
        ...
        while True:
            if first:
                hub.sleep(ECHO_DELAY*ECHO)#等待一段时间再进行拓扑发现
                first=False
            else:
    ...
    def _performance_pktin(self):
        ...
        while 1:
            if self.pkt_in_num!=0:
                ...
                pkts=self.pkt_in_num-self.old_pkt_in_num


                if pkts!=0:

                    delay = self.handler_delay/pkts

                else:

                    delay=0

                speed = pkts / PERFORMANCE_STATISTIC_ECHO

                for dp in self.switches_pktin_load.keys():

                    self.switches_pktin_load[dp]["speed"] = (self.switches_pktin_load[dp]["new"] - self.switches_pktin_load[dp]["old"]) / PERFORMANCE_STATISTIC_ECHO

                    self.switches_pktin_load[dp]["size"]=(self.switches_pktin_load[dp]["new_size"] - self.switches_pktin_load[dp]["old_size"])/ PERFORMANCE_STATISTIC_ECHO

                    self.switches_pktin_load[dp]["old"] = self.switches_pktin_load[dp]["new"]

                    self.switches_pktin_load[dp]["old_size"] = self.switches_pktin_load[dp]["new_size"]
    ...
    def _packet_in_handler(self,ev):
        ...
        if Eth_type == ether_types.ETH_TYPE_LLDP:
            ...
            self.handler_delay += (time.perf_counter_ns() - s) / 10 ** 6
    ...
    @set_ev_cls(ofp_event.EventOFPPacketIn,MAIN_DISPATCHER)
    def _statistic_pktin(self,ev):
        msg = ev.msg
        datapath = msg.datapath
        dpid = datapath.id
        if dpid not in self.switches_pktin_load.keys():

            self.switches_pktin_load.setdefault(dpid, {"new": 1, "old": 0, "speed": 0,"new_size":msg.total_len,"old_size":0,"size":0})
        else:
            self.switches_pktin_load[dpid]["new"] += 1

            self.switches_pktin_load[dpid]["new_size"] += msg.total_len
    # ===================================Path类======================================
    def shortest_forwarding(self,msg,ip_src,ip_dst):
        ...
        if result:
            ...
            if dst_sw:#如果目的主机存在本area
                ...
                port_no = self.find_shortest_path(ip_src, ip_dst, src_sw, dst_sw,to_dst_port)

                self.test_send_packet_out(datapath, msg.buffer_id, in_port, port_no, msg.data)
            else: #如果本地area不存在该目的IP，请求Server
                #self.log.info(f'IP:{ip_src}=>{ip_dst}外部area 请求Server')
                self.server_path(ip_src, ip_dst, src_sw, msg.data,in_port,msg.buffer_id)

    def find_shortest_path(self,ip_src,ip_dst,src_dpid,dst_dpid,dst_port):
        """
        ...
        """
        return self.local_path(ip_src,ip_dst,src_dpid,dst_dpid,dst_port)

    def server_path(self,ip_src,ip_dst,src_dpid,data,in_port,buffer_id):
        """
        ...
        cross_require=json.dumps({
            ...
            "data":{
                ...
                "ip_dst":ip_dst,
                "src_dpid": src_dpid,
                "msg_data":self.bytes_to_hexstr(data),
                "buffer_id":buffer_id,
                "in_port":in_port
            }
        })
    ...
    def local_path(self,ip_src,ip_dst,src_dpid,dst_dpid,dst_port):
        """
        {(ip_src,ip_dst):[path]} 路由表结构
        ...
        if src_dpid==dst_dpid:
            self.install_sw_to_host_flowmod(ether.ETH_TYPE_IP,src_dpid,ip_dst,dst_port)
    ...
    @staticmethod
    def test_add_flow(dp, p, match, actions, idle_timeout, hard_timeout):
        """
        测试pktin性能
    ...
    def test_send_packet_out(self, datapath, buffer_id, src_port, dst_port, data):
        """
           测试pktin性能
        """
        out = self.build_packet_out(datapath, buffer_id,
